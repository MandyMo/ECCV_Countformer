...
__all__ = ["VoVNet", "vovnet_backbone"]

_NORM = False

def get_norm(_NORM, out_channels):
    if _NORM:
...
class VoVNet(nn.Module):
    def __init__(self, cfg, input_ch=3, out_features=None):
        """
        Args:
...
def vovnet_backbone(NAME, NORM, out_features , in_ch=3, pretrained=True, progress=False, **kwargs):
    from easydict import EasyDict as edict
    cfg = edict(
        NAME = NAME,
        NORM = NORM,
    )
    model =  VoVNet(cfg, input_ch=in_ch, out_features=out_features)
    if pretrained:
        state_dict = load_state_dict_from_url('https://tri-ml-public.s3.amazonaws.com/github/dd3d/pretrained/depth_pretrained_v99-3jlw0p36-20210423_010520-model_final-remapped.pth', progress=progress)
        HELPERS.get('copy_state_dict')(model.state_dict(), state_dict['model'], prefix='backbone.bottom_up.')
